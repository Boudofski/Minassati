...
export function Header() {
  ...
  const primaryNav = [
    { href: prefix("/"), label: labels.home, icon: Home },
    { href: "/courses", label: labels.courses, icon: GraduationCap },
    { href: "/paths", label: labels.paths, icon: Layers },
    { href: "/articles", label: labels.articles, icon: Newspaper },
    { href: "/quran", label: labels.quran, icon: BookOpenCheck },
    { href: "/resources", label: labels.resources, icon: PackageOpen },
    { href: "/pricing", label: labels.pricing, icon: Tag },
    { href: "/instructors", label: labels.instructors, icon: Users },
  ];
  const moreItems = [
    { href: "/about", label: labels.about, icon: BookText },
    { href: "/contact", label: labels.contact, icon: Mail },
    { href: "/privacy", label: labels.privacy, icon: ShieldCheck },
    { href: "/terms", label: labels.terms, icon: BookText },
    { href: "/help", label: labels.help, icon: HelpCircle },
    { href: "/islamic-kids", label: "Islamic Kids", icon: BookOpenCheck },
  ];
      ...
      <div className="page-shell flex h-20 items-center justify-between gap-4">
        <Link href={prefix("/")} className="flex shrink-0 items-center gap-3" aria-label="منصتي">
          ...
          <span className="leading-tight">
            ...
            <span className="text-xs font-bold text-slate-500">تعلم، دورات، وموارد رقمية</span>
          </span>
        </Link>

        <nav className="hidden min-w-0 items-center gap-0.5 rounded-full border border-slate-200 bg-white p-1 shadow-sm xl:flex" aria-label="التنقل الرئيسي">
          {primaryNav.map((item) => {
        ...
        <div className="hidden items-center gap-2 xl:flex">
          <div className="relative" ref={dropdownRef}>
            <button onClick={() => setDropdownOpen((v) => !v)} className={cn("inline-flex items-center gap-1.5 rounded-full px-3 py-2 text-sm font-extrabold", dropdownOpen ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-100")}>
              {labels.more}<ChevronDown className={cn("h-4 w-4 transition", dropdownOpen && "rotate-180")} />
            </button>
            <AnimatePresence>
              {dropdownOpen && (
          ...
          <Link href="/courses" className="rounded-full bg-slate-950 px-4 py-2 text-sm font-black text-white">ابدأ التعلم</Link>
          <LanguageSwitcher />
        </div>

        <button onClick={() => setMobileOpen((v) => !v)} className="grid h-11 w-11 place-items-center rounded-2xl border border-slate-200 bg-white text-slate-800 xl:hidden" aria-label="القائمة">
